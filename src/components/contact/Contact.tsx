...
export default function Contact({ children }: Props) {
  const [selectedLocation, setLocation] = useState<number>(0)
  ...
  console.log("address[selectedLocation]", address[selectedLocation].iframeSrc)
...
{address[selectedLocation]?.iframeSrc && (

      <iframe
        src={address[selectedLocation]?.iframeSrc}
        width="600"
        height="450"
        style={{ border: 0 }}
        allowFullScreen
